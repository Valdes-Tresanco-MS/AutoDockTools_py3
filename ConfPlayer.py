...
#
# $Header: /opt/cvs/python/packages/share1.5/AutoDockTools/ConfPlayer.py,v 1.50.2.1 2016/02/11 09:24:06 annao Exp $
#
# $Id: ConfPlayer.py,v 1.50.2.1 2016/02/11 09:24:06 annao Exp $
#
#
...
#        return form
    ...
    def Build_cb(self, event=None):
        ...
        conf = self.sequenceList[confInd]
        self.buildConf(conf, str(numStr))


    def BuildAll_cb(self, event=None):
        """None<-BuildAll_cb(mol, event=None)
        adds a new molecule for each conf to the viewer
        """
        for conf in self.sequenceList:
            nStr = str(self.sequenceList.index(conf))
            self.buildConf(conf, str(nStr))
...
class PopulationPlayer(ConformationPlayer):
    ...
    def update(self, sequenceList=[], idList=[]):
        ...
        ph = self.docking.ph
        if len(sequenceList):
            assert(sequenceList in ph.all_populations)
            self.sequenceList = sequenceList
        else:
    ...
    def buildConf(self, conf, nameStr):
        nStr = str(self.sequenceList.index(conf))
        if hasattr(self.docking.dlo_list[0],'filename'):
            try:
                stem = os.path.splitext(os.path.basename(self.docking.dlo_list[0].filename))[0]
            except:
                stem = self.mol.name
        newname = stem + '_pop' + nStr
        if newname in self.vf.Mols.name:
            msg = newname + ' already in viewer. Not building a second copy'
            self.vf.warningMsg(msg)
            return 'ERROR'
        ...
        if isinstance(self.mol.parser, PdbqParser):
            ...
            pdbqParser = PdbqtParser() 
        pdbqParser.allLines = newLines
        #newname = self.mol.name + '_pop_'+nStr
        filename = pdbqParser.filename = newname + ".pdbqt"
        newMol = pdbqParser.parse()[0]          
        newMol.name = newname
        newMol = self.vf.addMolecule(newMol, ask=self.ask)
        newMol.colored = 0
