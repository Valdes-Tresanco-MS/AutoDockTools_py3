...
class WaterBuilder:
    ...
    def dist(self, firstline, secondline, precision=4):
        ...
        return round(sqrt((coord1[0] - coord2[0]) ** 2 + (coord1[1] - coord2[1]) ** 2 + (coord1[2] - coord2[2]) ** 2),
                     precision)
    ...
    def mean_pdb(self, firstline, secondline):
        ...
        mean_atom = "ATOM  %5d %4s %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00          %1s" % (
            count, atype, residue, self.chain, index, x, y, z, self.ATYPE)
    ...
    def mean3(self, firstline, secondline, thirdline):
        ...
        index = 1
        mean_atom = "ATOM  %5d %4s %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00          %1s" % (
            count, self.ATYPE, residue, chain, index, x, y, z, self.ATYPE)
        return mean_atom

    def hydro(self, atom1, atom2, atom3=None, spacing=3):
        # INFO   : place a water atom W at given self.distance from an atom INPUT  : (a) two pdb lines for N-H; (b)
        # three pdb lines for -[N|O]- acceptor, -O-H donors; the spacing self.distance between the water and the atom
        # OUTPUT : one pdb line
        #
        # Synopsis:
        #
            ...
            if candidate == atom or candidate == exclude:
                ...
                if candidate[0:4] == "ATOM" or candidate[0:6] == "HETATM":
                    ...
                    if self.dist(atom, candidate) <= bond_dist + tolerance:
                        if candidate not in bound_list:
    ...
    def calc_plane(self, atom1, atom2, atom3):
        ...
        if self.verbose:
            ...
            print("%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00     %1.3f %1s" % (
                self.keyw, index, self.ATYPE, residue, chain, index, plane[0], plane[1], plane[2], self.pcharge,
                self.ATYPE))
    ...
    def Osp2(self, oxygen, atom1, atom2):
        ...
        for a in angles:
            ...
            # wet = "%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00     %1.3f %1s\n"%(self.keyw, 1, self.ATYPE,
            # residue, chain, 1, water[0], water[1], water[2], self.pcharge, self.ATYPE)
            waters.append(wet)
        if self.verbose:
    ...
    def hydrate(self, pdbqt, gpf=None, force=False, extended_atoms=False, output=None):
        ...
        if output is not None:
            if os.path.isdir(output):
                if self.verbose:
                    print(" - saving the file in the path => %s" % output)
                name = output + os.path.sep + name + "_HYDRO.pdbqt"
            else:
                if self.verbose:
                    print(" - saving the file => %s" % output)
                name = output
        ...
        for atom in hydrate_list:
            ...
            if atype == "HD":
                # check for errors
                if len(master) > 1:
                    print("\n\nERROR (HD) : there is a proximity error and the following hydrogen is in close "
            ...
            if atype == "OA":
                ...
                if len(master) == 1:
                    ...
                    if len(mates) <= 2:
                        ...
                        # print "CARBONYL"
                        # print "%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00     %1.3f %1s" % (keyw, index,
                        # ATYPE, residue, chain, index, plane[0], plane[1], plane[2], pcharge, ATYPE)
                        ...
                        wet = "%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00     %1.3f %1s\n" % (
                            ...
                            self.ATYPE)
                        # wet = "%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00     %1.3f %1s\n" % (self.keyw,
                        # 1, self.ATYPE, self.residue, self.chain, 1, wat[0], wat[1], wat[2], self.pcharge, self.ATYPE)
                        if self.verbose:
                            print("O1: wet:")
                            print(wet)
                        waters_generated.append(wet)
                        ...
                        roto = [plane0, plane1, plane2, radians(-50)]
                        if self.verbose:
                            print("calling rotatePoint with self.atom_coord(", atom, "), roto=", roto)
                        wat = self.rotatePoint(self.normalize(-v12) * self.space, self.atom_coord(atom), roto)
                        if self.verbose:
                            print("wat =", wat)
                        wet = "%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00     %1.3f %1s\n" % (
                        ...
                        if self.EXTENDED_ATOMS:
                            ...
                            # if self.verbose: print "%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00
                            # %1.3f %1s\n" % (keyw, 1, self.ATYPE, residue, chain, 1, directive[0], directive[1],
                            # directive[2], pcharge, self.ATYPE)
                            ...
                            for q in mates:
                                ...
                                wet = "%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00     %1.3f %1s\n" % (
                                    self.keyw, 1, self.ATYPE, residue, chain, 1, lpair[0], lpair[1], lpair[2],
                                    self.pcharge,
                                    self.ATYPE)
                                # wet = "%s%5d %2s   %3s %1s%4d    %8.3f%8.3f%8.3f  1.00 10.00     %1.3f %1s\n" %
                                # (self.keyw, 1, self.ATYPE, self.residue, self.chain, 1, lpair[0], lpair[1], lpair[2],
                                # self.pcharge, self.ATYPE)
                                if self.verbose:
                                    print("EA: wet=", end=' ')
                                    for ind in wet:
                                        print(ind)
