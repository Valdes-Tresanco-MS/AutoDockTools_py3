...
#                                                                                                  #
#  Modification date: 4/5/20 12:54                                                                 #
#                                                                                                  #
# ##################################################################################################
...
class Psize:
    def __init__(self):
        self.constants = {"CFAC": 1.7, "FADD": 20, "SPACE": 0.50, "GMEMFAC": 200, "GMEMCEIL": 400, "OFAC": 0.1,
                          "REDFAC": 0.25, "TFAC_ALPHA": 9e-5, "TFAC_XEON": 3e-4, "TFAC_SPARC": 5e-4}
    ...
    def printResults(self):
        ...
        if self.gotatom > 0:
            ...
            text = text + "################# MOLECULE INFO ####################\n"
            text = text + "Number of ATOM entries = %i\n" % self.gotatom
            text = text + "Number of HETATM entries (ignored) = %i\n" % self.gothet
            text = text + "Total charge = %.3f e\n" % q
            text = text + "Dimensions = %.3f x %.3f x %.3f A\n" % (olen[0], olen[1], olen[2])
            text = text + "Center = %.3f x %.3f x %.3f A\n" % (cen[0], cen[1], cen[2])
            text = text + "Lower corner = %.3f x %.3f x %.3f A\n" % (minlen[0], minlen[1], minlen[2])
            text = text + "Upper corner = %.3f x %.3f x %.3f A\n" % (maxlen[0], maxlen[1], maxlen[2])

            text = text + "\n"
            text = text + "############## GENERAL CALCULATION INFO #############\n"
            text = text + "Coarse grid dims = %.3f x %.3f x %.3f A\n" % (clen[0],
                                                                         clen[1], clen[2])
            text = text + "Fine grid dims = %.3f x %.3f x %.3f A\n" % (flen[0], flen[1], flen[2])
            text = text + "Num. fine grid pts. = %i x %i x %i\n" % (n[0], n[1], n[2])

            if gmem > self.constants["GMEMCEIL"]:
                text = text + "Parallel solve required (%.3f MB > %.3f MB)\n" % (gmem, self.constants["GMEMCEIL"])
                text = text + "Total processors required = %i\n" % (np[0] * np[1] * np[2])
                text = text + "Proc. grid = %i x %i x %i\n" % (np[0], np[1], np[2])
                text = text + "Grid pts. on each proc. = %i x %i x %i\n" % (nsmall[0], nsmall[1], nsmall[2])
                ...
                if np[2] == 1:
                    zglob = nsmall[2]
                text = text + "Fine mesh spacing = %g x %g x %g A\n" % (
                    flen[0] / (xglob - 1), flen[1] / (yglob - 1), flen[2] / (zglob - 1))
                text = text + "Estimated mem. required for parallel solve = %.3f MB/proc.\n" % nsmem
                ntot = nsmall[0] * nsmall[1] * nsmall[2]

            else:
                text = text + "Fine mesh spacing = %g x %g x %g A\n" % (
                    flen[0] / (n[0] - 1), flen[1] / (n[1] - 1), flen[2] / (n[2] - 1))
                text = text + "Estimated mem. required for sequential solve = %.3f MB\n" % gmem
                ntot = n[0] * n[1] * n[2]

            text = text + "Number of focusing operations = %i\n" % nfocus

            text = text + "\n"
            text = text + "################# ESTIMATED REQUIREMENTS ####################\n"
            text = text + "Memory per processor                   = %.3f MB\n" % (200.0 * ntot / 1024 / 1024)
            text = text + "Grid storage requirements (ASCII)      = %.3f MB\n" % (
                    8.0 * 12 * np[0] * np[1] * np[2] * ntot / 1024 / 1024)
            text = text + "Grid storage requirements (XDR)        = %.3f MB\n" % (
                    8.0 * ntot * np[0] * np[1] * np[2] / 1024 / 1024)
            text = text + "Time to solve on 667 MHz EV67 Alpha    = %.3f sec\n" % tsolve_alpha
            text = text + "Time to solve on 500 MHz PIII Xeon     = %.3f sec\n" % tsolve_xeon
            text = text + "Time to solve on 400 MHz UltraSparc II = %.3f sec\n" % tsolve_sparc
            text = text + "\n"

        else:
            text = text + "No ATOM entires in file!\n\n"
...
def main():
    ...
    filename = ""
    shortOptList = ""
    longOptList = ["help", "CFAC=", "FADD=", "SPACE=", "GMEMFAC=", "GMEMCEIL=", "OFAC=", "REDFAC=", "TFAC_ALPHA=",
                   "TFAC_XEON=", "TFAC_ALPHA="]
    try:
