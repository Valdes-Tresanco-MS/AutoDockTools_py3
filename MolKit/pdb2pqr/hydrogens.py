...
class hydrogenRoutines:
    ...
    def switchstate(self, states, amb, id):
        ...
        for conf in hdef.conformations:
            ...
            if residue.getAtom(hname) is not None:
    ...
    def optimizeSingle(self, amb):
        ...
        # Brute force for flips
            ...
            raise ValueError("Invalid Hydrogen type %i in %s %i!" % (type, residue.get("name"), residue.get("resSeq")))
        ...
        for cluster in networks:
            ...
            if len(cluster) == 1:
                ...
                if len(cluster) == 2:
                    ...
                    steps = 15
                elif len(cluster) >= 4 and len(cluster) < 10:
                    steps = pow(2, len(cluster))
                if steps > 200 or len(cluster) >= 10:
                    steps = 200
    ...
    def liststates(self):
        ...
        for i in range(len(self.groups)):
            ...
            if residue.get("isNterm"):
                ...
                elif H2 is not None or H3 is not None:
                ...
                if HD1 is not None and HE2 is not None:
                    state = "Positive HIS"
                elif HD1 is not None:
                    state = "HIS HD1"
                elif HE2 is not None:
                    state = "HIS HE2"
                else:
                    state = "Negative HIS"
                ...
                if HD1 is not None or HD2 is not None:
                    state = "Neutral ASP"
                else:
                    state = "Negative ASP"
                ...
                HE2 = residue.getAtom("HE2")
                if HE1 is not None or HE2 is not None:
                    state = "Neutral GLU"
                else:
                    state = "Negative GLU"
                ...
                HE2 = residue.getAtom("HE2")
                if HE1 is not None:
                    state = "Neutral GLU (HE1)"
                elif HE2 is not None:
                    state = "Neutral GLU (HE2)"
                else:
                    raise ValueError("GLH should always be neutral!")
    ...
    def getHbondEnergy2(self, clusteratoms, compatoms, res=None):
        ...
        if res is not None:
            for atom2 in compatoms:
                ...
                if res2 != res:
                    continue
                elif not atom2.get("hacceptor"):
                    continue
                for atom1 in clusteratoms:
                    if atom1.get("residue") == res2:
                        continue
                    elif not atom1.get("hdonor"):
                        continue
                    elif distance(atom1.getCoords(), atom2.getCoords()) < HYDROGEN_DIST:
                        pair = self.getPairEnergy(atom1, atom2)
                        energy = energy + pair
                        # print "\tComparing %s %i to %s %i %.2f" % (atom1.name, atom1.residue.resSeq, atom2.name,
                        # atom2.residue.resSeq, pair)

        for atom1 in clusteratoms:
            energy = energy + self.getPenalty(atom1)
            res1 = atom1.get("residue")
            if res is not None and res1 != res:
                continue
            elif not res1.get("isNterm") and atom1.get("name") == "N":
                continue
            elif not atom1.get("hdonor"):
                continue
            for atom2 in compatoms:
                if res1 == atom2.get("residue"):
                    continue
                elif not atom2.get("hacceptor"):
                    continue
                elif distance(atom1.getCoords(), atom2.getCoords()) < HYDROGEN_DIST:
    ...
    def getHbondEnergy(self, amb):
        ...
        for nearatom in nearatoms:
            for atom in residue.get("atoms"):
                if atom.get("hdonor"):
                    if not nearatom.get("hacceptor"):
                        continue
                    elif atom.get("name") == "N" and not residue.get("isNterm"):
                        continue
                    elif nearatom.get("name") == "O" and not nearatom.residue.get("name") == "WAT":
                        continue
                    if distance(atom.getCoords(), nearatom.getCoords()) < HYDROGEN_DIST:
                        pair = self.getPairEnergy(atom, nearatom)
                        energy = energy + pair
                if atom.get("hacceptor"):
                    if not nearatom.get("hdonor"):
                        continue
                    elif atom.get("name") == "O" and not residue.get("name") == "WAT":
                        continue
                    elif nearatom.get("name") == "N" and not nearatom.residue.get("isNterm"):
                        continue
                    if distance(atom.getCoords(), nearatom.getCoords()) < HYDROGEN_DIST:
    ...
    def getPenalty(self, residue):
        ...
        if residue.get("isNterm"):
            charge = 1
            if residue.getAtom("H2") is None:
                charge = charge - 1
            if residue.getAtom("H3") is None:
                charge = charge - 1
        ...
        if resname == "HIS":
            ...
            elif hd1 is not None and he2 is not None:
    ...
    def randomizeWaters(self):
        ...
        for residue in waters:
            ...
            for atom in allatoms:
                ...
                if oxygen != atom and closedist < WATER_DIST:
                    closeatoms[residue].append(atom)
        ...
        for residue in waters:
            # print "Optimizing WAT %i" % residue.resSeq
            oxygen = residue.getAtom("O")
            if self.watermap[residue] is not None:
                continue
            bestdon = None
            bestacc = None
            bestdondist = 999.99
    ...
    def optimizeWaters(self):
        ...
        for residue in waters:
            # print "Optimizing WAT %i" % residue.resSeq
            oxygen = residue.getAtom("O")
            if self.watermap[residue] is not None: continue
            bestdon = None
            bestacc = None
            bestdondist = 999.99
            bestaccdist = 999.99
    ...
    def printAmbiguities(self):
        ...
        for amb in self.groups:
            ...
            self.routines.write(
                "Ambiguity #: %i, chain: %s, residue: %i %s, hyd_type: %i state: %i Grp_name: %s Hname: %s "
                "Boundatom: %s\n" % (i, residue.chainID, residue.resSeq, residue.name, hydrodef.type,
                                     hydrodef.standardconf, hydrodef.name, conf.hname, conf.boundatom))
            i += 1
    ...
    def parseHydrogen(self, lines):
        ...
        mydef = HydrogenDefinition(name, group, numhydrogens, standardconf, type, chiangle, method)

        conf = []
        for newline in lines[1:]:
            if newline.startswith(">"):
                if conf == []: continue
                confinfo = conf[0].split()
                hname = confinfo[0]
