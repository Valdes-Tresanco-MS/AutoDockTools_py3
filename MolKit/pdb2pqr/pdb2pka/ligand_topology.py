...
#                                                                                                  #
#  Modification date: 4/5/20 13:56                                                                 #
#                                                                                                  #
# ##################################################################################################
...
class get_ligand_topology:
    ...
    def __init__(self, lines, MOL2FLAG):
        ...
        if MOL2FLAG == False:
            ...
            for line in lines:
                splited = line.split()
                name = splited[0]
                self.atoms[name] = {'coords': np.array([float(splited[1]), float(splited[2]), float(splited[3])])}
            ...
            for line in lines:
                ...
                self.atoms[name] = {'coords': np.array([float(line.x), float(line.y), float(line.z)])}
    ...
    def matched_atom_types(self, atom2match, t):
        match_list = []
        #
                ...
                if matched_atom_in_template != None:
                    for entries in matched_atom_in_template:
                        ...
                        for sybyl_bonded_at in self.atoms[at_lig]['lBondedAtoms']:
                            ...
                            if len(ligand_set.difference(template_set)) == 0 and len(ligand_list) == len(
                                    nbs_in_template):
                                for entry in matched_atom_in_template:
                                    print("%3d" % (counter), "  Ligand %4s %5s %28s " \
                                          % (at_lig, self.atoms[at_lig]['sybylType'], ligand_list), \
                                          "template %s %s %s %s" \
                                          % (matched_atom_in_template, t[entry]['sybylType'], nbs_in_template,
                                             t[entry]['neighbours']))
                                    for neighboured_template_atoms in t[entry]['neighbours']:
                                        print(neighboured_template_atoms, t[neighboured_template_atoms]['sybylType'],
                                              t[neighboured_template_atoms]['sybyl_neighbours'])
                                    for neighboured_ligand_atoms in self.atoms[at_lig]['lBondedAtoms']:
                                        print(neighboured_ligand_atoms.name, neighboured_ligand_atoms.sybylType,
                                              neighboured_ligand_atoms.lBondedAtoms)
                                    stop
                counter += 1
            ...
            for ent_lig in stored_nbs_of_atom2match:
                ...
                if ent_lig not in already_visited:
                    ...
                    for matches in matchlist:
                        if len(set(t[matches]['sybyl_neighbours']).difference(
                                set(createsybyllistonthefly(ent_lig)))) == 0:
