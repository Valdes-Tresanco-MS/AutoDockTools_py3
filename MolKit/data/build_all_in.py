...
def getDUMMList(i, allLines):
    ...
    if ll[1] == 'DUMM':
        ...
        DUMMLIST.append(ll)
        DUMMLIST.append(allLines[i + 1].split())
        DUMMLIST.append(allLines[i + 2].split())
        i = i + 3
    return DUMMLIST, i
...
def buildRes(i, allLines):
    ...
    while not endAtoms:
        if len(allLines[i].strip().split()) < 9:
            endAtoms = 1
            if ifixc == 'CHANGE' or len(allLines[i + 1].strip().split()) == 0:
                i = i + 1
        else:
            l = allLines[i].strip()
            if not len(l):
                i = i + 1
            atName, newD[atName] = getAtom(allLines[i].strip().split())
            atNameList.append(atName)
        i = i + 1

    ll = allLines[i].strip().split()
    i = i + 1
...
def buildResDict(inFile='all_amino94.in'):
    ...
    f.close()
    outdict = {}
    outdict['IDBGEN,IREST,ITYPF'] = allLines[0].split()
    outdict['NAMDBF'] = allLines[1].strip()
    i = 2
    done = 0
    while not done:
...
def write(_dict, outfile='test_dat.py', dname='all_amino94_dat'):
    fptr = open(outfile, 'w')
    outstr = dname + ' = {\n'
    fptr.write(outstr)
    # NB each v is also a dictionary
    for k, v in _dict.items():
        if type(v) == type(_dict):
            fptr.write('\"' + k + '\"' + ': {')
...
_dict = unint_dat = buildResDict('unint.in')
write(_dict, outfile='unint_dat.py', dname='unint_dat')
_dict = opls_unint_dat = buildResDict('opls_unint.in')
write(_dict, outfile='opls_unint_dat.py', dname='opls_unint_dat')
_dict = opls_nacl_dat = buildResDict('opls_nacl.in')
write(_dict, outfile='opls_nacl_dat.py', dname='opls_nacl_dat')
_dict = unict_dat = buildResDict('unict.in')
write(_dict, outfile='unict_dat.py', dname='unict_dat')
_dict = opls_unict_dat = buildResDict('opls_unict.in')
write(_dict, outfile='opls_unict_dat.py', dname='opls_unict_dat')
_dict = uni_dat = buildResDict('uni.in')
write(_dict, outfile='uni_dat.py', dname='uni_dat')
_dict = opls_uni_dat = buildResDict('opls_uni.in')
write(_dict, outfile='opls_uni_dat.py', dname='opls_uni_dat')
_dict = nh2e_dat = buildResDict('nh2e.in')
write(_dict, outfile='nh2e_dat.py', dname='nh2e_dat')
_dict = allnt_dat = buildResDict('allnt.in')
write(_dict, outfile='allnt_dat.py', dname='allnt_dat')
_dict = allct_dat = buildResDict('allct.in')
write(_dict, outfile='allct_dat.py', dname='allct_dat')
_dict = all_dat = buildResDict('all.in')
write(_dict, outfile='all_dat.py', dname='all_dat')
_dict = all_nuc94_dat = buildResDict('all_nuc94.in')
write(_dict, outfile='all_nuc94_dat.py', dname='all_nuc94_dat')
_dict = all_aminoct94_dat = buildResDict('all_aminoct94.in')
# write(_dict, outfile='test_all_aminoct94_dat.py',dname='all_aminoct94_dat')
write(_dict, outfile='all_aminoct94_dat.py', dname='all_aminoct94_dat')
_dict = all_aminont94_dat = buildResDict('all_aminont94.in')
write(_dict, outfile='all_aminont94_dat.py', dname='all_aminont94_dat')
_dict = all_amino94_dat = buildResDict('all_amino94.in')
write(_dict, outfile='all_amino94_dat.py', dname='all_amino94_dat')
