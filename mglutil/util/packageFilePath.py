...
resourceFolder = ''
if home:
    resourceFolder = os.path.join(home, ".mgltools")
...
def findResourceFile(module, resourceFile=None):
    ...
    if resourceFile is None:
        ...
        resourceFileLocation = {'package': None, 'currentdir': None}
        lResourceFolder = getResourceFolderWithVersion()
        if lResourceFolder is None:
            resourceFileLocation['home'] = None
        else:
            resourceFileLocation['home'] = getResourceFolderWithVersion() + os.sep + 'Vision' + os.sep + '_visionrc'
...
"""
    ...
    if os.path.isdir(resourceFolder) is False:
        try:
            ...
            txt = "Resource Folder can't be created in home directory, now using tmp directory %s %s" % (
                resourceFolder, e)
            warnings.warn(txt)
