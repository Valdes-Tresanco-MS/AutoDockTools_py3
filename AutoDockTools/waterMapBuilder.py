...
class WaterMapBuilder:
    ...
    def __init__(self, receptor_file=None, OAmap_file=None, HDmap_file=None,
        ...
        if receptor_file is None and (OAmap_file is None or HDmap_file is None):
            raise Exception("receptor filename or both oxygen map file AND hydrogen map file must be specified")
        #???
        self.O = 0
        self.H = 0
        ...
        if output_stem is None:
            raise Exception("output_stem is None!")
        ...
        if self.OAmap_file is None:
            raise Exception("oxygen map file not specified")
        if self.HDmap_file is None:
            raise Exception("hydrogen map file not specified")
...
#        DIFFERENCE.close
        if mode in ['best', 'best_w']:
            ...
            H = float(self.H)
            tot = H+O
            if tot == 0: raise Exception('O+H == zero !')
            o_pc = (O/tot)*100
            h_pc = (H/tot)*100
