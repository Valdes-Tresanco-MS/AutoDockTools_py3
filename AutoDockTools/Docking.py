        ...
        if receptor:
            ...
            if parser.isV1:
                if len(parser.lig_close_ats) and ligand.name in parser.lig_close_ats[0]:
                    ...
                    close_ats, close_ats_str = self.css.select(ligandSet, parser.lig_close_ats[0].strip())
                    if not len(close_ats):
                        print("skipping ligand close ats because ", parser.lig_close_ats[0],
                              " did not match atoms in the ligand")
                ...
                if hasattr(parser, 'macro_close_ats') and len(parser.macro_close_ats) and receptor.name in \
                        parser.macro_close_ats[0]:
                    ...
                    for pi_str in parser.pi_cation:
                        ...
                        if ok:
                            ...
                            if len(lig_ats) == 1:
                                ...
                                print("skipping pi_str because ", lig_str,
                                      " multiple atoms specified for both lig + rec")
                                continue
                            else:
            ...
            if top is not None:
                ...
                if parser.isV1:
                    ...
                    ligand.clustNB = InteractiveHistogramGraph(ligand.name,
                                                               ...
                                                               label_text=ligand.name + ':' + rms + ' rms',
                                                               xlabel_text=xlabel,
                                                               ylabel_text='#\nC\nO\nN\nF\nO\nR\nM\nA\nT\nI\nO\nN\nS')
    ...
    def write_current_conformation(self, filename="", infoStr=None, rms=-1, ncl_to_write=-1, summary_only=False):
        ...
        if infoStr and hasattr(self, 'clusterer') and len(self.clusterer.clustering_dict):
            ...
            if infoStr is None:
                infoStr = "#"
            # ?IS THIS EVER CALLED??
            sss, dr_str = self.clusterer.getInfoStr(comment=infoStr, ind=best_ind, rms=rms, ncl_to_write=ncl_to_write,
                                                    include_dlgfilename_run=True)
            sss_list = sss.split('\n')
...
class FoxResultProcessor:
    ...
    def Dist(self, f, s, sq=True):
        ...
        if sq:
            return sqrt((float(f[30:38]) - float(s[30:38])) ** 2 + (float(f[38:46]) - float(s[38:46])) ** 2 + (
                    float(f[46:54]) - float(s[46:54])) ** 2)
        else:
            return (float(f[30:38]) - float(s[30:38])) ** 2 + (float(f[38:46]) - float(s[38:46])) ** 2 + (
                    float(f[46:54]) - float(s[46:54])) ** 2
...
CODE TO REPLACE THESE LINES USED FOR TESTING AND DEVELOPMENT:
    ...
    pdbqt = frp.createPDBQTplus(results, total_runs, histogram, receptor_filename, ligand_name, dlg_list = dlg_list)
        ...
        pdbqt = self.createPDBQTplus(results, total_runs, histogram, receptor_filename, ligand_name, dlg_list=dlg_list,
                                     clusters=clusters)
    ...
    def createPDBQTplus(self, results, total_runs, histogram, receptor_filename, lig_mol_name, dlg_list,
                        max_dlg_count=None, clusters=[]):
        """
        ...
        for p in range(len(results)):
            ...
            obuff += "USER   AD_%s> %2.3f,\t%2.3f,\t%d,\t%3.2f\n" % (
                tag[p], results[p][0], self.lig_eff(pose[0], len(self.atype_list)), pose[1],
                self.percent(pose[1], total_runs))

            # add hb acceptors info
            if hb_dictionary['acceptors']:
                obuff += "USER   AD_%s_hba> " % (tag[p])
                for pair in hb_dictionary['acceptors']:
                    obuff += "%s~~%s," % (self.pmv_atom_stripper(lig_mol_name, pair[0], contract=True),
                                          self.pmv_atom_stripper(self.rec_mol_name, pair[1], contract=True))
                obuff = obuff[:-1] + "\n"
...
class DockingResultProcessor:
    ...
    def process(self, directory=None, outputfilename=None, verbose=False, rms_tolerance=None, dlg_list=[]):
        ...
        try:
            ...
            if deduced_name:
                print(
                    "Sorry unable to locate '%s' as deduced from gridmap names.\nPlease specify complete path to receptor file after '-r' " % (
                        self.receptor_filename))
            else:
        ...
        if self.write_both or self.best_only or index_LC == 0:
            ...
            sss, dr_str = d.clusterer.getInfoStr(comment='USER  AD> ', ind=best_ind, rms=rms_tolerance,
                                                 ncl_to_write=max_cl_to_write, include_dlgfilename_run=True)
            sss_list = sss.split('\n')
            ...
            for line in d.ligMol.parser.allLines:
                ...
                if line.find("HETATM") == 0 or line.find("ATOM") == 0:
                    nl = line[:30] + "%8.3f%8.3f%8.3f" % (
                        d.ligMol.allAtoms[atm_ct].coords[0], d.ligMol.allAtoms[atm_ct].coords[1],
                        d.ligMol.allAtoms[atm_ct].coords[2]) + line[54:]
                    atm_ct += 1
                    # nl = line
                    be_ptr.write(nl + "\n")
                else:
            ...
            if self.include_interactions:
                ...
                if verbose: print("calling self.intF.processLigand with be_outfilename=", be_outfilename, "opf=",
                                  outputfilename)
        ...
        if (self.write_both or self.largestCl_only) and index_LC != 0:
            ...
            if self.include_interactions:
                ...
                outputfilename = os.path.join(directory, outputfilename)
                if verbose: print("calling self.intF.processLigand with lc_outfilename=", lc_outfilename, "opf=",
                                  outputfilename)
...
class VinaResultProcessor:
    ...
    def process(self, vina_resultfile, outputfilestem=None, remove_model_str=False, best_only=None, verbose=False):
        ...
        for i in range(num_to_write):
            ...
            self.intD.processLigand(outputfilename, outputfilename=outputfilename, buildHB=0,
                                    remove_modelStr=remove_modelStr)
            optr = open(outputfilename, 'r')
            lines = optr.readlines()
            optr.close()
