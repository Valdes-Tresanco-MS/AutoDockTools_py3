...
#
#
# $Header: /opt/cvs/python/packages/share1.5/AutoDockTools/Utilities24/prepare_covalent_flexres.py,v 1.7.6.2 2016/02/11 09:24:08 annao Exp $
#
import os
...
def detect_clashes(atomset1, atomset2, cutoff=1.2, verbose=False):  #check this v. autodock/mdist.h
    ...
    clashes = []
    for at1 in atomset1:
        if verbose: print("processing ", at1.name, end=' ')
        bonded = getBondedAtoms(at1)
        if verbose: print("len(bonded)=", len(bonded), end=' ')
        for at2 in atomset2:
...
def main():
    ...
    if nsc==0:
        ...
        raise Exception("line 274 ERROR")
    ...
    rM.allAtoms.number = list(range(1, num_ats+1))
